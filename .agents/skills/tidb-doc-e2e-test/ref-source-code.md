...
4. Resolve the documentation's target version to the relevant source branch, tag, or commit and record the exact commit inspected. Component versions and repository layouts can differ from the TiDB version: follow the target release's dependencies and source history instead of assuming identical tags or today's repository layout. Prefer `git show <revision>:<path>` for committed source without switching the user's checkout. Treat local uncommitted changes as evidence only when the user asks to inspect them, and record that distinction.
